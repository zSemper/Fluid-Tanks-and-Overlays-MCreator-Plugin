    ...
    <#if blockEntity != "">
        public void addFluidWidget() {
            if (getMenu().boundBlockEntity != null) {
                <#list data.getComponentsOfType("FluidTank") as component>
                    addRenderableOnly(new FluidStackWidget(this, getMenu().boundBlockEntity.getFluidTank${component.id}(), this.leftPos + ${component.gx(data.width)}, this.topPos + ${component.gy(data.height)}, ${component.width}, ${component.height},
                        <#if hasProcedure(component.tooltip)>
                            <@procedureOBJToConditionCode component.tooltip />
                        <#else>
                            null
                        </#if>
                    ));
