...
public class ${name}Menu extends AbstractContainerMenu implements ${JavaModName}Menus.MenuAccessor {
	...
	<#if blockEntity != "">
	    public ${blockEntity.getModElement().getName()}BlockEntity boundBlockEntity = null;
	<#else>
		...
		<#if data.type == 1>
			if (pos != null) {
				if (extraData.readableBytes() == 1) { // bound to item
				    ...
				    <#if blockEntity != "">
				        boundBlockEntity = (${blockEntity.getModElement().getName()}BlockEntity) this.world.getBlockEntity(pos);
				    <#else>
